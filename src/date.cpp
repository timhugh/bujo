...
const std::vector<char> regex_metacharacters = {
    '.', '^', '$', '|', '(', ')', '[', ']', '{', '}', '*', '+', '?', '\\'};

void append_escaped_literal(std::string &out, char c) {
...
void append_capture(std::string &out, std::string_view pattern) {
  out.append("(");
  out.append(pattern);
  out.append(")");
}

compiled_template compile(std::string_view fmt) {
  std::string pattern;
  pattern.push_back('^');
...
capture_map match(const compiled_template &ct, const std::string &input) {
  std::cmatch m;
...
::date::local_days resolve_day(const capture_map &cm) {
  if (!cm.year) {
...
::date::local_days floor_to_week_start(::date::local_days d, week_start ws) {
...
::date::local_days resolve_iso_week(const capture_map &cm) {
  if (!cm.iso_year) {
    if (cm.year) {
      throw std::invalid_argument(
...
::date::local_days resolve_month(const capture_map &cm) {
  if (!cm.year) {
    throw std::invalid_argument(
...
::date::local_days resolve_year_start(const capture_map &cm) {
  ::date::year y{*cm.year};
...
::date::local_days resolve_iso_year_start(const capture_map &cm) {
  ::date::year_month_day jan4{::date::year{*cm.iso_year} / ::date::January / 4};
  if (!jan4.ok()) {
    throw std::invalid_argument(
        fmt::format("Invalid ISO year: {}", *cm.iso_year));
...
::date::local_days resolve_span_start(const capture_map &cm) {
  if (cm.day) {
